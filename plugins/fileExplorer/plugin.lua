local function init(components)
			...
			elseif not showingControls then
				...
				elseif key == keys.enter then
					...
					elseif fs.isDir(fullPath) then
						...
						TextBuffer.filename = fullPath
						TextBuffer:loadFile(fullPath)
						...
						if not fs.exists(newPath) then
							if string.sub(newName, -1) == "/" then
								...
								if file then
									...
									fileExplorerWindow:print("File creation failed!")
								end
							end
							refreshFiles()
						else
				...
				elseif key == keys.d then
					...
					if selectedFile == ".." then
						...
						local fullPath = fs.combine(shell.dir(), selectedFile)
						local confirmation = captureInput(
							"Type 'CONFIRM' to delete '" .. selectedFile .. "' (or press Enter to cancel): "
						)
				...
				elseif key == keys.r then
					...
					if selectedFile == ".." then
						...
						local fullPath = fs.combine(shell.dir(), selectedFile)
						local newName =
							captureInput("Enter new name for '" .. selectedFile .. "' (or press Enter to cancel): ")
						if newName and newName ~= "" then
							local newPath = fs.combine(shell.dir(), newName)
							if not fs.exists(newPath) then
	...
	InputController:map({ "normal" }, { "backslash" }, "open_file_explorer", function()
		openFileExplorer()
